...
<!--                </template>-->
              ...
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="座机号码">
                <a-input placeholder="请输入座机号码" v-model="model.mobile" />
              </a-form-model-item>
              <a-form-model-item  :labelCol="labelCol" :wrapperCol="wrapperCol"  label="地址">
                <a-row :gutter="16">
                  <a-col :span="12">
                    <j-area-linkage v-model="model.departNameEn" type="cascader"  />
                  </a-col>
                  <a-col :span="12">
                    <a-input placeholder="请输入街道和门牌号" v-model="model.address" />
                  </a-col>
                </a-row>
              </a-form-model-item>
    ...
    data() {
      return {
        ...
        validatorRules: {
          ...
          mobile:[{validator: this.validateLandline}]
        },
        url: {
    ...
    methods: {
     ...
     //---- author:os_chengtgen -- date:20190827 --  for:切换父子勾选模式 =======------
      ...
      validateLandline(rule,value,callback){
        if (!value || new RegExp(/^\d{3}-\d{7,8}|\d{4}-\d{7,8}$/).test(value)){
          callback();
        }else{
          callback("您的座机号码格式不正确!");
        }
